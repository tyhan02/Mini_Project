...
x = datasets[['id', 'bus_route_id', 'in_out', 'station_code', 'station_name',
              ...
              '8~9_takeoff', '9~10_takeoff']].copy() #11~12 뺌
...
# x = x.drop(['6~7_takeoff','7~8_takeoff'], axis=1)

# # x['takeoff_avg_8~10'] = (x['8~9_takeoff'] + x['9~10_takeoff']) / 2
# # x = x.drop(['8~9_takeoff','9~10_takeoff'], axis=1)
...
# # x['takeon_avg_6~12'] = (x['6~7_ride'] + x['7~8_ride'] + x['8~9_ride'] + x['9~10_ride'] ) /4 
...
x['takeon_avg_6~12'] = (x['10~11_ride'] + x['11~12_ride']) / 2
...
tf.random.set_seed(100) # weight 난수값 조정
...
#     x, y, test_size=0.13, train_size=0.87, random_state=80, shuffle=True
# ) # 7:3에서 바꿈
# print(x)
...
x_train, x_test, y_train, y_test = train_test_split(
    x, y, test_size=0.10, train_size=0.9, random_state=80, shuffle=True
) # 7:3에서 바꿈
print(x)
